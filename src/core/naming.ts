...
const KIND_PATTERNS: Array<[TaskKind, RegExp]> = [
	...
	["explore", /(?:^\s*(?:please\s+)?(?:explore|investigate|research|plan|design|audit|review)\b|\b(?:spike|prototype)\b)/i],
...
export function heuristicName(prompt: string, config: AutoRenameConfig): NameProposal {
	...
	const cleaned = sentence.replace(LEADING_FILLER, "").replace(/[.!?:;,]+$/g, "").trim() || "Task";
